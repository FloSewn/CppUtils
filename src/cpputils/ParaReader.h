...
public:
  ...
  *-----------------------------------------------------------------
  ...
  {
    // Read the file content
    content_ = get_content( file_path );

    // Set bounds of top ParaBlock
    para_block_.block_start( 0 );
  ...
  *------------------------------------------------------------------
  ...
  template <typename T>
  void new_scalar_parameter(const string& name, const string& key)
  { para_block_.new_scalar_parameter<T>(name, key); } 

  template <typename T>
  void new_vector_parameter(const string& name, 
                            const string& key, size_t n)
  { para_block_.new_vector_parameter<T>(name, key, n); }
  ...
  *------------------------------------------------------------------
  ...
  ParaBlock& get_parameter(const string& name)
  { return para_block_.get_parameter<ParaBlock>( name ); }

  template <typename T>
  T get_value(const string& name)
  { return para_block_.get_value<T>( name ); }

  template <typename T>
  T get_value(size_t i, const string& name)
  { return para_block_.get_value<T>( i, name ); }
  ...
  *------------------------------------------------------------------
  ...
  template <typename T>
  bool query(const string& name)
  { return para_block_.query<T>( name, content_ ); }

  bool query(const string& name)
  { return para_block_.query( name, content_ ); }
